...
class QLearnig:
    ...
    def initialize_parameters(self):
        ...
        self.current_site = self.find_site_matched_condition(map=self.map, condition=1)
        self.trap_sites = self.find_site_matched_condition(map=self.map, condition=-1)
        self.treasure_site = self.find_site_matched_condition(map=self.map, condition=2)
        self.reach_terminal = False
        self.path = []
    ...
    def move(self, action):
        ...
        if movable_flag:
            movable_flag &= not self.visit_by_indexList(self.map, new_coordinate) == -1
            if movable_flag:
    ...
    def advanced_move(self):
        ...
        for ind, actn in enumerate(self.actions):
            ...
            if self.tensor_equal(next_site, self.current_site) or \
                            self.visit_by_indexList(self.map, next_site) == -1:
                pass
            else:
        ...
        if np.random.uniform() > self.epsilon or self.tensor_equal(self.current_site, [0] * self.dim):
            ...
            subTensor = np.asarray([subTensor[i] if i in action_ind else -1 for i in xrange(len(subTensor))])
